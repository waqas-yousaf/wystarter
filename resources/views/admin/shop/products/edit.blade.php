<x-layouts.admin title="Edit Product">
    <form method="POST" action="{{ route('admin.shop.products.update', $product) }}" class="max-w-2xl space-y-5">
        @csrf @method('PUT')
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="4"
                          class="block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500">{{ old('description', $product->description) }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input id="stock_qty" name="stock_qty" type="number" min="0" value="{{ old('stock_qty', $product->stock_qty) }}" required
                           class="block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500">
                </div>
            </div>
