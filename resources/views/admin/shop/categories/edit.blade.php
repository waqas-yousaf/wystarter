<x-layouts.admin title="Edit Category">
    <form method="POST" action="{{ route('admin.shop.categories.update', $category) }}" class="max-w-lg space-y-5">
        @csrf @method('PUT')
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3"
                          class="block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500">{{ old('description', $category->description) }}</textarea>
            </div>
