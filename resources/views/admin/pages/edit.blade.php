        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
            ...
            <div class="lg:col-span-2 space-y-5">
                ...
                <div>
                    ...
                    <x-editor name="body" :value="old('body', $page->body ?? '')" />
                    @error('body')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="space-y-5">
                ...
                <div class="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
                    <div class="flex items-center justify-between">
                        ...
                        @if ($page->published_at)
                            <span class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium bg-green-100 text-green-800">Published</span>
                        @else
                    ...
                    <label class="flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
                        <input type="checkbox" name="published_at" value="1"
                               {{ old('published_at', $page->published_at ? '1' : '') ? 'checked' : '' }}
                               class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                        Published
                    </label>
                    @if ($page->published_at)
                        <p class="text-xs text-gray-400">Since {{ $page->published_at->format('M j, Y') }}</p>
                    @endif
                    ...
                    @if ($page->published_at)
                        <a href="{{ route('page.show', $page->slug) }}" target="_blank"
                           class="flex items-center justify-center gap-1.5 w-full rounded-lg border border-gray-200 px-3 py-2 text-xs text-gray-500 hover:text-indigo-600 hover:border-indigo-300">
                ...
                <div class="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
                    ...
                    <div>
                        ...
                        <textarea id="meta_description" name="meta_description" rows="3" maxlength="300"
                                  class="block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500">{{ old('meta_description', $page->meta_description) }}</textarea>
                ...
                <div class="bg-white rounded-xl border border-gray-200 p-4 text-xs text-gray-500 space-y-1">
                    <p>Created {{ $page->created_at->format('M j, Y H:i') }}</p>
                    <p>Updated {{ $page->updated_at->format('M j, Y H:i') }}</p>
                </div>
            </div>
        </div>
    </form>
